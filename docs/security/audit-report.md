...
- **Меры (resolved/mitigated):**
  ...
  2. `pip install --upgrade pip setuptools>=78.1.1 wheel>=0.46.2`
     перетирает vendored `wheel` / `jaraco.context`-копии setuptools
     актуальными версиями.
  3. `build-essential` удаляется из prod stage через `apt-get purge
     --auto-remove`, поэтому build-only `linux-libc-dev` не попадает в
     финальный backend image.
  4. Финальный образ запускается под непривилегированным uid 1000
     ...
     primary-vector.
  5. `.trivyignore` временно содержит `CVE-2026-40355` для Debian
     `krb5`-пакетов (`libgssapi-krb5-2`, `libk5crypto3`, `libkrb5-3`,
     `libkrb5support0`). GitHub Actions Trivy SARIF помечает этот
     finding как `MEDIUM`, но action всё равно возвращает non-zero
     при `exit-code: 1`, хотя gate настроен на `HIGH,CRITICAL`.
     Исключение нужно удалить, как только runner/base image подтянет
     `1.21.3-5+deb13u1` или Trivy перестанет учитывать Medium в этом
     job-е.
