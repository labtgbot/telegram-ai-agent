...
export function humanContentError(err: unknown, fallback = "Request failed."): string {
  if (isApiError(err)) {
    ...
    if (err.status === 404) return "Item no longer exists.";
    if (err.status === 409) {
      const payload = err.payload as { detail?: unknown } | undefined;
      if (typeof payload?.detail === "string") return payload.detail;
      return "Duplicate code — pick a different identifier.";
    }
    if (err.status === 400) {
      const payload = err.payload as { detail?: unknown } | undefined;
      if (typeof payload?.detail === "string") return payload.detail;
